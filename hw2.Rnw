...
<<data, echo=FALSE, cahce=FALSE, comment=NA, results='asis', warning=FALSE>>=
...
for(j in 1:n){
  ...
  CVn_j[j] <- (air$C[j] - mean(air$C[-j]))^2 #intercept only model, prediction for all X is mean(air$C)
}
CVn <- mean(CVn_j)
